...
// ProxyErrorHeader marks errors generated by the Agent Vault proxy so clients
// can distinguish them from upstream responses with the same status code.
const ProxyErrorHeader = "X-Agent-Vault-Proxy-Error"

const proxyErrorMessagePrefix = "Agent Vault: "
...
func ForbiddenHintBody(targetHost, vaultName, baseURL string) map[string]interface{} {
	body := map[string]interface{}{
		"error":   "forbidden",
		"message": proxyErrorMessagePrefix + fmt.Sprintf("No broker service matching host %q in vault %q", targetHost, vaultName),
...
func writeProxyErrorWithHelp(w http.ResponseWriter, status int, code, message, baseURL string) {
	...
	w.Header().Set(ProxyErrorHeader, "true")
	w.WriteHeader(status)
	body := map[string]interface{}{"error": code, "message": proxyErrorMessagePrefix + message}
	if baseURL != "" {
