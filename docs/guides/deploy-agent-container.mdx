...
Run `agent-vault agent token renew` inside a single-owner agent runtime to replace its current token without a management credential. For a mounted token file, use `agent-vault agent token renew --token-file /run/secrets/agent-vault-token`; it atomically replaces the file with owner-only permissions. A replacement takes effect immediately, so coordinate the runtime's reload with the token file update.

Use `agent-vault agent rotate <name>` from an operator session when the runtime lost the renewal response, a competing process won a renewal, or compromise is suspected.
