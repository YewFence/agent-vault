...
The unit needs Agent Vault's environment wired explicitly, since systemd doesn't inherit it from your shell. Before creating the unit, generate the current CA installation script with `agent-vault ca install-script --address http://10.0.0.2:14321 > agent-vault-ca-install.sh`, review it, and run its Linux `install` action so the gateway can use native system trust.
