...
- **Mounts**:
  ...
  - `<data-dir>/isolation/ca-<session>.pem → /etc/agent-vault/ca.pem` (read-only, MITM CA; data directory defaults to `~/.agent-vault`)
...
| `--mount` | | Extra bind mount `src:dst[:ro]`; repeatable. Host paths are symlink-resolved before validation; binds into, containing, or pointing at the configured Agent Vault data directory or `/var/run/docker.sock` are rejected. |
