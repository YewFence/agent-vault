...
<Tabs>
  <Tab title="vault run (simplest)">
    ...
    `vault run` sets `AGENT_VAULT_ACTIVE=true`, `AGENT_VAULT_ADDR`, `AGENT_VAULT_TOKEN`, and `AGENT_VAULT_VAULT` on the child, then pre-configures `HTTPS_PROXY`/`HTTP_PROXY` and the CA trust chain so standard HTTP clients route both HTTP and HTTPS traffic through the broker transparently.
  </Tab>
...
| Variable | Purpose |
|---|---|
| `AGENT_VAULT_ACTIVE` | Set to `true` to identify a process launched by `agent-vault vault run` |
