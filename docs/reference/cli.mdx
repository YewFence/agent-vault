...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    | `--mitm-port` | `14322` | Port for the transparent MITM proxy. Enabled by default; set to `0` to disable. On first launch the root CA is created under the configured data directory (`~/.agent-vault/ca/` by default) — clients fetch it with [`agent-vault ca fetch`](#ca). Bind failures are non-fatal. |
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_HOME` | Dedicated local data directory (default `~/.agent-vault`) shared by server and CLI state. Relative paths are resolved from the process working directory. Existing data is not copied when this value changes. `owner reset` deletes the entire directory. |
    ...
    | `OTEL_EXPORTER_OTLP_ENDPOINT` | Enables proxy metrics, per-request logs, and traces export to an OTLP Collector. If unset (and no per-signal endpoint is set), OpenTelemetry export is disabled. See [Observability](/self-hosting/observability). |
    | `OTEL_EXPORTER_OTLP_PROTOCOL` | OTLP transport for all signals: `grpc` (default) or `http/protobuf`. Production endpoints should use TLS. |
    | `OTEL_EXPORTER_OTLP_HEADERS` | Authentication and other OTLP exporter headers. Treat this value as a secret. |
    | `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` | Per-signal override sending only metrics elsewhere; enables metrics even without the generic endpoint. `OTEL_EXPORTER_OTLP_METRICS_PROTOCOL` and `OTEL_EXPORTER_OTLP_METRICS_HEADERS` follow the same pattern. |
    | `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` | Per-signal override sending only request logs elsewhere; enables logs even without the generic endpoint. `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL` and `OTEL_EXPORTER_OTLP_LOGS_HEADERS` follow the same pattern. |
    | `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` | Per-signal override for traces; enables traces without the generic endpoint. `OTEL_EXPORTER_OTLP_TRACES_PROTOCOL` and `OTEL_EXPORTER_OTLP_TRACES_HEADERS` follow the same pattern. |
    | `OTEL_TRACES_SAMPLER` | Standard OTel sampler; default `parentbased_always_on` follows parent sampling and samples new traces. Supports `always_on`, `always_off`, `traceidratio`, and their `parentbased_` variants. Unknown sampler names fall back to the default with an SDK diagnostic. |
    | `OTEL_TRACES_SAMPLER_ARG` | Ratio from `0` to `1` for ratio-based samplers; defaults to `1` when unset. Malformed or out-of-range ratios fall back to `1` with an SDK diagnostic, retaining the selected sampler's parent behavior. Does not affect logs or metrics. |
    | `AGENT_VAULT_TRACE_PROPAGATION_ALLOWLIST` | Startup-only comma-separated hostnames or `*.example.com` subdomain patterns. Empty/invalid means no outbound W3C context propagation. While tracing is enabled, allowed targets receive trace context and retain baggage; other targets have `traceparent`, `tracestate`, and `baggage` removed. Does not restrict export to the Collector. Disabled tracing or failed initialization preserves existing forwarding. |
    | `OTEL_SERVICE_NAME` | Overrides the OpenTelemetry service name (default `agent-vault`). |
    | `OTEL_RESOURCE_ATTRIBUTES` | Additional OpenTelemetry resource attributes. |
    | `OTEL_SDK_DISABLED` | Set to `true` or `1` to disable OpenTelemetry export (metrics, logs, and traces) even when an endpoint is configured. Trace propagation policy is also disabled, preserving existing request-header forwarding. |
  ...
  <Accordion title="agent-vault server stop">
    ...
    Stop a running server. Reads `agent-vault.pid` from the configured data directory and sends SIGTERM.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="agent-vault migrate-db">
    ...
    | `--from` | `<data-dir>/agent-vault.db` | Source SQLite database path. The data directory defaults to `~/.agent-vault` and respects `AGENT_VAULT_HOME`. |
...
<AccordionGroup>
  <Accordion title="agent-vault ca fetch">
    ...
    **Examples:**
    ...
    # For native trust-store installation, generate a reviewable script instead.
    agent-vault ca install-script > agent-vault-ca-install.sh
    ```
  </Accordion>

  <Accordion title="agent-vault ca install-script">
    ```bash
    agent-vault ca install-script [--address URL] > agent-vault-ca-install.sh
    ```

    Fetch the current CA and print a reviewable platform-native script to stdout. The command does not execute the script, change a trust store, or elevate privileges. Save it, inspect the embedded certificate fingerprint and commands, then run the reviewed file manually. Do not pipe this command directly to a shell.

    On Linux the script requires p11-kit `trust anchor` and `trust extract-compat`; it does not detect distributions. On macOS it presents explicit login-keychain and System-keychain actions. Each script can install, verify, or remove the exact embedded certificate.
  </Accordion>

  <Accordion title="agent-vault ca verify">
    ```bash
    agent-vault ca verify [--file ca.pem] [--address URL]
    ```

    Verify the current CA, or the PEM supplied with `--file`, against the native system trust policy. `vault run` performs the same check before starting a host child process and prints the installation-script command if it fails.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault auth sessions revoke">
    ...
    Revoke a specific session by id (from `auth sessions list`). Use this to invalidate a stolen `session.json` from the configured data directory or to log out a forgotten device. Revoking your current session forces you to log in again on this device.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault run (alias: agent-vault vault run)">
    ...
    `vault run` sets `AGENT_VAULT_ACTIVE=true` on the child so it can reliably identify this Agent Vault-managed process. In host mode, `vault run` first requires the current Agent Vault CA to be installed in the native system trust store. It then passes `HTTPS_PROXY` / `HTTP_PROXY`, their lowercase equivalents, matching `NO_PROXY` / `no_proxy`, `NODE_USE_ENV_PROXY`, `OPENCLAW_PROXY_URL`, `UV_SYSTEM_CERTS=true`, and append-only `NODE_EXTRA_CA_CERTS`. It deliberately clears replacement-style variables such as `SSL_CERT_FILE`, `REQUESTS_CA_BUNDLE`, `CURL_CA_BUNDLE`, `GIT_SSL_CAINFO`, and `DENO_CERT` so the child retains public system roots. If verification fails, save and review `agent-vault ca install-script` before retrying, or use `--isolation=container`. Container mode installs the CA in the image trust store and points file-oriented clients at its complete system bundle. `OPENCLAW_PROXY_URL` feeds OpenClaw's Proxyline managed proxy (OpenClaw requires this plus `proxy.enabled: true` in its config). `HTTPS_PROXY` and `HTTP_PROXY` both point at the same plain HTTP proxy URL — the listener handles CONNECT for `https://` upstreams and absolute-form forward-proxy requests for `http://` upstreams on the same port. If the server's MITM proxy is unreachable, `vault run` aborts.

    **Placeholder env delivery.** For every substitution with an explicit `env` setting on the vault's enabled services, the child additionally receives an env var holding the placeholder (e.g. `GITHUB_TOKEN=github_pat_thisisaplaceholder00…0`) — a spec-shaped fake value that passes SDK/static format checks and that the broker rewrites to the real credential on the wire. Substitutions without `env` do not affect the child environment. Parent-environment values for configured names are scrubbed first so a real credential set outside the vault never leaks into the child. If enabled services assign different placeholders to the same env name, the run aborts because one environment variable cannot represent both values. Fetching the service list failing also aborts the run. See [Credential-shaped placeholders](/learn/services#credential-shaped-placeholders-and-env-delivery).
    ...
    **Agent mode (containerized / unattended deployments).** When `AGENT_VAULT_TOKEN` and `AGENT_VAULT_ADDR` are pre-set on the environment, `vault run` skips the admin-session login and uses the env-supplied token as the credential — `--ttl` is rejected in this mode since the token's lifetime is fixed at mint time. The token is validated against the broker once at startup so bad/expired tokens fail fast with a clear error rather than producing 401s on every proxied call. See [Deploy your agent in a container](/guides/deploy-agent-container).
    ...
    | `--no-skills` | `false` | Skip automatic skill installation or updates for recognized agents. Also respects `AGENT_VAULT_NO_SKILLS`; an explicitly supplied flag takes precedence. Applies to host-mode skill-file installation only. |
    | `--telemetry` | `true` | Enable or disable anonymous usage telemetry. Also respects the `AGENT_VAULT_TELEMETRY` env var; the env var takes precedence. |
  </Accordion>

  <Accordion title="agent-vault placeholders">
    ```bash
    agent-vault placeholders [flags]
    ```

    Print portable POSIX shell exports for placeholders whose enabled service substitutions explicitly configure `env`. The output is sorted by environment variable name and contains placeholder values only; warnings and errors go to stderr, so successful output is safe to evaluate in `sh`, Bash, or Zsh.

    The shell must already have `AGENT_VAULT_TOKEN`, `AGENT_VAULT_ADDR`, `AGENT_VAULT_VAULT`, proxy routing, and CA trust configured. If enabled services assign different placeholders to the same environment variable, the command stops without emitting a script.

    | Flag | Default | Description |
    |------|---------|-------------|
    | `--address` | | Server address override |
    | `--vault` | | Target vault override |
  ...
  <Accordion title="agent-vault placeholders generate">
    ```bash
    agent-vault placeholders generate
    ```

    Interactively mask a real API key or token into a credential-shaped placeholder of the same length: vendor prefix + `thisisaplaceholder` + zero padding. The credential is read with hidden input and never leaves the terminal — no server connection, session, or vault is needed. The prefix prompt is prefilled with the longest matching catalog prefix (pasting an `sk-or-…` key suggests `sk-or-`); accept it or type your own.

    Standard output contains only the placeholder, so it is safe to capture with command substitution; notes and warnings go to stderr.

    ```bash
    GITHUB_TOKEN_PLACEHOLDER=$(agent-vault placeholders generate)
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault agent token renew">
    ```bash
    agent-vault agent token renew [--token-file path]
    ```

    Uses `AGENT_VAULT_ADDR` and the current `AGENT_VAULT_TOKEN` to replace the token for the same agent identity. With no flag, stdout contains only the replacement token. `--token-file` reads the current token from the file and atomically replaces it with a mode-`0600` replacement token.

    The command is for agent runtimes. Use `agent-vault agent rotate <name>` with a management session if renewal loses its response or another process has already renewed the token.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault owner reset">
    ...
    Permanently delete all data and reset the instance to a fresh state. Requires a running server for role verification. Auto-stops the server before wiping the configured data directory, whose absolute path is shown in the confirmation prompt. Owner only.
...
| `AGENT_VAULT_HOME` | `~/.agent-vault` | Data directory checked for a running server and an existing SQLite database during upgrades. Use the same value used to run Agent Vault. |
