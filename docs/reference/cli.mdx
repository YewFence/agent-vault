...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `OTEL_EXPORTER_OTLP_ENDPOINT` | Enables proxy metrics and per-request logs export to an OTLP Collector. If unset (and no per-signal endpoint is set), OpenTelemetry export is disabled. See [Observability](/self-hosting/observability). |
    ...
    | `OTEL_SDK_DISABLED` | Set to `true` or `1` to disable OpenTelemetry export (metrics and logs) even when an endpoint is configured. |
