...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `OTEL_EXPORTER_OTLP_ENDPOINT` | Enables proxy Metrics export to an OTLP Collector. If unset, Metrics export is disabled. |
    | `OTEL_EXPORTER_OTLP_PROTOCOL` | OTLP transport: `grpc` (default) or `http/protobuf`. Production endpoints should use TLS. |
    ...
    | `OTEL_SDK_DISABLED` | Set to `true` or `1` to disable OpenTelemetry Metrics even when an endpoint is configured. |
