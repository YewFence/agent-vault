...
User sessions are **long-lived but idle-expired**. A login is good for up to a year of activity; if a session goes 30 days without making a request, it is automatically rejected as expired. Active operators effectively never have to log in again, while a stolen `session.json` from the configured data directory (`~/.agent-vault` by default) that goes unused loses access in 30 days. Operators can view and revoke individual sessions with `agent-vault auth sessions list / revoke`, or invalidate every session at once via `account change-password`.
...
Outbound proxy connections are validated against the resolved IP. By default, every RFC-1918, loopback, link-local, IPv6 ULA, and CGN range is blocked, plus cloud metadata endpoints. A rejected proxy request returns HTTP `403` with error code `ssrf_blocked`, and its request log row records the same error code instead of treating the denial as an upstream failure. Set `AGENT_VAULT_ALLOW_PRIVATE_RANGES=true` for local/trusted deployments where the proxy needs to reach internal services on private IPs. To keep the block on but punch narrow holes for known internal services, set `AGENT_VAULT_NETWORK_ALLOWLIST=10.163.0.0/16,192.168.1.1` (CIDRs or bare IPs, comma-separated). Cloud metadata endpoints stay blocked regardless.
