...
<Tabs>
  <Tab title="Shell">
    ...
    After you install the current Agent Vault CA into the host's native trust store with a saved and reviewed `agent-vault ca install-script`, `agent-vault run` automatically configures OpenClaw's managed proxy (`proxy.enabled`) and trusted env proxy for `web_fetch`, sets `OPENCLAW_PROXY_URL`, `HTTPS_PROXY`, `HTTP_PROXY`, and the system-trust environment, and installs the Agent Vault skill. No manual OpenClaw configuration needed.
