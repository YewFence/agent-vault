...
The simplest approach for local development. Install the current CA into the native host trust store using a saved and reviewed `agent-vault ca install-script`, then wrap a local agent process with the environment variables it needs. `vault run` pre-configures `HTTPS_PROXY`/`HTTP_PROXY` and preserves system CA trust on the child, so the agent calls upstream URLs directly (over `https://` or `http://`) and Agent Vault transparently injects credentials at the proxy boundary.
...
### Renewing and rotating an agent token

An agent runtime can replace its own current token without storing a management session:

```bash
export AGENT_VAULT_TOKEN="$(agent-vault agent token renew)"
```

For a persistent runtime, keep the token in a mode-`0600` file. The command writes and syncs a temporary file in the same directory, then atomically replaces the token file:

```bash
agent-vault agent token renew --token-file ~/.config/agent-vault/my-agent.token
```

Only one process should own an agent token. If a renewal response is lost or another process renews first, the old token cannot recover the replacement; use an operator rotation instead.

```bash
agent-vault agent rotate my-agent
```

Operator rotation invalidates the current token and prints a new one. It is the recovery path for a lost renewal response or suspected token compromise.
