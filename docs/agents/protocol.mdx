...
Agent Vault agents follow a standard HTTP protocol to interact with the server. Agents handle this automatically — the canonical reference lives in the [agent skill directory](https://github.com/Infisical/agent-vault/tree/main/skills/agent-vault-cli). Its main instructions are also served at `/v1/skills/agent-vault-cli/SKILL.md`, with proposal instructions at `/v1/skills/agent-vault-cli/references/proposals.md`. This page is a reference for understanding what happens under the hood.
...
When `agent-vault vault run` starts the agent process, it additionally sets
`AGENT_VAULT_ACTIVE=true` to identify the Agent Vault-managed runtime.
...
| Variable | Purpose |
|---|---|
| `HTTPS_PROXY`, `https_proxy` | Point at the MITM listener (`http://{token}:{vault}@host:14322`) — plain HTTP, deploy on a trusted/private network |
...
| `NO_PROXY`, `no_proxy` | Include `localhost`, `127.0.0.1`, and the Agent Vault control-plane hostname |
| `NODE_USE_ENV_PROXY` | `1` — enables Node.js v22.21+ built-in proxy support for `fetch()` and `http.get()`/`https.get()` |
| `UV_SYSTEM_CERTS` | `true` — makes uv use the native system certificate verifier |
| `NODE_EXTRA_CA_CERTS` | Adds the Agent Vault CA for Node without replacing public roots |

Before host-mode `vault run`, install the current Agent Vault CA in the native trust store with a saved and reviewed `agent-vault ca install-script`. Replacement-style CA variables are cleared so `NO_PROXY` traffic and public HTTPS endpoints retain normal system-root validation. Container mode installs the CA inside its image and uses a merged system bundle for file-oriented clients.
...
| 403 `ssrf_blocked` | The destination resolved to an address blocked by Agent Vault's network policy | Surface the policy denial to the user; don't retry unless an operator changes the private-range policy or network allowlist. |
