...
| `AGENT_VAULT_HOME` | Optional (defaults to `~/.agent-vault`) | Dedicated directory for local Agent Vault state, including SQLite, sessions, CA files, PID files, logs, and isolation assets. Relative paths are resolved from the process working directory. Set this before every server and CLI invocation that should use the same local state. Existing data is not copied automatically. `agent-vault owner reset` deletes this entire directory, so never point it at a directory containing unrelated files. |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Optional | Enables Agent Vault proxy metrics, per-request logs, and traces, exported to the configured OTLP Collector endpoint. See [Observability](/self-hosting/observability). If unset (and no per-signal endpoint is set), OpenTelemetry export is disabled. |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | Optional (defaults to `grpc`) | OTLP transport for all signals: `grpc` or `http/protobuf`. Production endpoints should use TLS. |
| `OTEL_EXPORTER_OTLP_HEADERS` | Optional | Authentication and other headers for the OTLP exporter. Treat this value as a secret. |
| `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` | Optional | Per-signal override sending only metrics to a different collector. Setting it enables metrics even without the generic endpoint. The same override pattern applies to `OTEL_EXPORTER_OTLP_METRICS_PROTOCOL` and `OTEL_EXPORTER_OTLP_METRICS_HEADERS`. |
| `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` | Optional | Per-signal override sending only request logs to a different collector. Setting it enables logs even without the generic endpoint. The same override pattern applies to `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL` and `OTEL_EXPORTER_OTLP_LOGS_HEADERS`. |
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` | Optional | Per-signal override for traces; enables traces without the generic endpoint. `OTEL_EXPORTER_OTLP_TRACES_PROTOCOL` and `OTEL_EXPORTER_OTLP_TRACES_HEADERS` override the generic transport and exporter headers. |
| `OTEL_TRACES_SAMPLER` | Optional (defaults to `parentbased_always_on`) | Standard OTel sampler: `always_on`, `always_off`, `traceidratio`, `parentbased_always_on`, `parentbased_always_off`, or `parentbased_traceidratio`. Unknown sampler names fall back to `parentbased_always_on` with an SDK diagnostic. Logs and metrics are not trace-sampled. |
| `OTEL_TRACES_SAMPLER_ARG` | Optional | Ratio from `0` to `1` for ratio-based samplers (defaults to `1` when unset). Malformed or out-of-range ratios fall back to `1` with an SDK diagnostic, retaining the selected sampler's parent behavior. Other samplers do not need an argument. |
| `AGENT_VAULT_TRACE_PROPAGATION_ALLOWLIST` | Optional (empty) | Startup-only comma-separated destination hostnames, with optional `*.example.com` subdomain patterns. Empty or invalid lists deny all outbound W3C context propagation while tracing is enabled. Allowed targets receive trace context and retain caller baggage; others have `traceparent`, `tracestate`, and `baggage` removed. Does not filter trace export. No effect when tracing is disabled or initialization fails. See [propagation rules](/self-hosting/observability#outbound-propagation). |
| `OTEL_SERVICE_NAME` | Optional (defaults to `agent-vault`) | Overrides the OpenTelemetry service name. |
| `OTEL_RESOURCE_ATTRIBUTES` | Optional | Additional OpenTelemetry resource attributes, such as `deployment.environment=production`. |
| `OTEL_SDK_DISABLED` | Optional | Set to `true` or `1` to disable OpenTelemetry export (metrics, logs, and traces) even when an OTLP endpoint is configured. Also disables trace propagation policy, preserving existing request-header forwarding. |
...
| `AGENT_VAULT_NO_SKILLS` | Optional (defaults to `false`) | Skip automatic skill installation or updates for recognized agents launched by `agent-vault run`. Overridden by an explicit `--no-skills` or `--no-skills=false`. |
