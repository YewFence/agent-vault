...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Optional | Enables Agent Vault proxy metrics and per-request logs, exported to the configured OTLP Collector endpoint. See [Observability](/self-hosting/observability). If unset (and no per-signal endpoint is set), OpenTelemetry export is disabled. |
...
| `OTEL_SDK_DISABLED` | Optional | Set to `true` or `1` to disable OpenTelemetry export (metrics and logs) even when an OTLP endpoint is configured. |
