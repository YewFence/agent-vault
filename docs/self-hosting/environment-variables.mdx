...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Optional | Enables Agent Vault proxy metrics and exports them to the configured OTLP Collector endpoint. If unset, metrics export is disabled. |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | Optional (defaults to `grpc`) | OTLP transport: `grpc` or `http/protobuf`. Production endpoints should use TLS. |
...
| `OTEL_SDK_DISABLED` | Optional | Set to `true` or `1` to disable OpenTelemetry metrics even when an OTLP endpoint is configured. |
