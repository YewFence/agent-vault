...
A software-backed root CA is created on first launch under `~/.agent-vault/ca/` (private key encrypted with the DEK). Clients must trust this root before the proxied TLS handshake will succeed. On macOS and Linux, host-mode `agent-vault vault run` requires that CA in the native system trust store before it starts its child process.
