...
This runs the multi-stage build in `Dockerfile`. The builder installs the locked mise toolchain, builds the React/Vite frontend, and compiles the Go binary; the final stage copies the binary into the minimal Alpine runtime image.
...
By default, all state lives in a single SQLite database at `/data/.agent-vault/agent-vault.db`. The Docker image declares `VOLUME /data`, so data survives container restarts as long as you mount a named volume or host path. `AGENT_VAULT_HOME` can override the data directory, but the configured path must remain inside a persistent mount if the state should survive container replacement.
